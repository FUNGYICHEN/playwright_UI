...
const { userRecordKey, userRecordValue } = require('./Q6constants');
const { randomHongKongPhoneNumber, randomUsername } = require('./phoneNumbers');
const { fetchVerificationCode } = require('./Q6phonecode');// 引入验证码获取函数 // 引入验证码获取函数
...
test('檢查個人頁icon圖片', async () => {
    ...
    const iconsToCheck = [
        ...
        { text: '合營代理', class: 'icon-moneyback', url: 'http://wap-q6-npf2.qit1.net/res/images/com-q6/account-center/icon-friend.png' },
    ...
    for (const { text, class: iconClass, url } of iconsToCheck) {
        ...
        if (iconExists) {
            ...
            console.log(`${text} 背景圖片: ${backgroundImage}`);
        }

        // 斷言背景圖片是否符合預期
        if (backgroundImage !== `url("${url}")`) {
            errors.push(`${text} 背景圖片不正確，預期: url("${url}")，實際: ${backgroundImage}`);
        }
        ...
        console.log(`${text} 圖片大小: 寬度=${width}px, 高度=${height}px`);

        // 斷言圖片大小是否正確
        if (width !== 27 || height !== 27) {
            errors.push(`${text} 圖片大小不正確，寬度=${width}px, 高度=${height}px`);
        }
        ...
        try {
            ...
            console.log(`${text} 圖片狀態碼: ${statusCode}`);
            console.log(`${text} 圖片文件大小: ${fileSize} bytes`);

            if (statusCode !== 200) {
                errors.push(`${text} 圖片加載失敗，狀態碼: ${statusCode}`);
            }
            if (fileSize === 0) {
                errors.push(`${text} 圖片文件大小為0`);
            }
        } catch (error) {
            errors.push(`${text} 圖片加載失敗`);
        }
    }
    ...
    // 確保沒有錯誤
    expect(errors.length, `以下是檢測到的錯誤: ${errors.join(', ')}`).toBe(0);
});
...
test('檢查關於港體會', async () => {
    ...
    console.log(`港體會icon 是否存在: ${logoVisible}`);
    if (logoVisible) {
        ...
        console.log(`港體會icon大小: ${logoSize.width}x${logoSize.height}`);
    ...
    // 檢查並點擊“關於 港體會”按鈕
    const aboutButton = await page.locator('.main-label .main-label-text', { hasText: '關於 港體會' }).first();
    const aboutButtonVisible = await aboutButton.isVisible();
    console.log(`關於 港體會按鈕是否存在: ${aboutButtonVisible}`);
    if (!aboutButtonVisible) {
        missingElements.push('關於 港體會按鈕');
    } else {
    ...
    const textsToCheck = [
        ...
        '港體會致力於忠誠與可信賴的博彩保證。我們公司遵從遠端博彩管理當局的適用法規以及指引，而且努力成為對社會負責任的遠端博彩運營公司。遠程博彩是全球數以百萬玩家的合法娛樂體驗。對大多數玩家來說，遠端博彩是一項令人愉快的體驗，不過，我們也接受這樣的現實，少部分沈迷在遠端博彩的玩家可能會未達到法定年齡或者出現由於博彩而影響了他們的生活或財務狀況的問題。作為一個對社會負責的公司意味著要關注我們的玩家，意味著要對可能對社會產生影響的問題採用主動的方法去解決。這正是為何港體會會採用並完全承諾執行以下最嚴格的程式和限制。',
        '執行政策',
        '對未成年人的訪問限制',
...
test('檢查關於港體會(EN)', async () => {
    ...
    // 檢查並點擊“關於 港體會”按鈕
    const aboutButton = await page.locator('.main-label .main-label-text', { hasText: 'About Kong Lotto' }).first();
    const aboutButtonVisible = await aboutButton.isVisible();
    console.log(`關於 About Kong Lotto按鈕是否存在: ${aboutButtonVisible}`);
    if (!aboutButtonVisible) {
        missingElements.push('關於 About Kong Lotto');
    } else {
        await aboutButton.click();
    }
