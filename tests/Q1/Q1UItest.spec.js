...
test.describe('@WAP Q1 測試', () => {
    ...
    test('檢查關於betone', async () => {
        ...
        // 導航到個人頁面
        await page.goto('http://wap-q1.qbpink01.com/accountCenter');
        await page.waitForLoadState('networkidle');
        ...
        console.log(`betone icon 是否存在: ${logoVisible}`);
        if (logoVisible) {
            ...
            console.log(`betone icon大小: ${logoSize.width}x${logoSize.height}`);
        ...
        // 檢查並點擊“關於 betone”按鈕
        const aboutButton = await page.locator('.main-label .main-label-text', { hasText: '關於 betone' }).first();
        const aboutButtonVisible = await aboutButton.isVisible();
        console.log(`關於 betone按鈕是否存在: ${aboutButtonVisible}`);
        if (!aboutButtonVisible) {
            missingElements.push('關於 betone按鈕');
        } else {
        ...
        const textsToCheck = [
            '博彩責任',
            '本平台致力於忠誠與可信賴的博彩保證。我們公司遵從遠端博彩管理當局的適用法規以及指引，而且努力成為對社會負責任的遠端博彩運營公司。遠程博彩是全球數以百萬玩家的合法娛樂體驗。對大多數玩家來說，遠端博彩是一項令人愉快的體驗，不過，我們也接受這樣的現實，少部分沈迷在遠端博彩的玩家可能會未達到法定年齡或者出現由於博彩而影響了他們的生活或財務狀況的問題。作為一個對社會負責的公司意味著要關注我們的玩家，意味著要對可能對社會產生影響的問題採用主動的方法去解決。這正是為何本平台會採用並完全承諾執行以下最嚴格的程式和限制。',
            '執行政策',
            '對未成年人的訪問限制',
        ...
        for (const expectedText of textsToCheck) {
            const textVisible = await page.locator('body').evaluate((body, expectedText) => {
                return body.innerText.includes(expectedText);
            }, expectedText);
            console.log(`文案 "${expectedText}" 是否存在: ${textVisible}`);
            if (!textVisible) missingElements.push(`文案 "${expectedText}"`);
        }
    ...
    test('檢查about betone (EN)', async () => {
        ...
        const logoVisible = await logo.isVisible();
        console.log(`betone icon visible: ${logoVisible}`);
        if (logoVisible) {
            const logoSize = await logo.boundingBox();
            console.log(`betone icon size: ${logoSize.width}x${logoSize.height}`);
            const expectedWidth = 20;
            const expectedHeight = 20;
            if (logoSize.width !== expectedWidth || logoSize.height !== expectedHeight) {
        ...
        const aboutButtonVisible = await aboutButton.isVisible();
        console.log(`About betone button visible: ${aboutButtonVisible}`);
        if (!aboutButtonVisible) {
    ...
    const textsToCheck = [
        ...
        { text: '', tag: 'div.about-desc' },
        { text: 'Execution policy', tag: 'div.about-heading' },
        { text: 'Access restrictions for minors', tag: 'div.about-desc' },
        { text: '', tag: 'div.about-desc' },
        { text: `Our platform will not allow anyone under the age of 18 to use our services. This policy fully complies with and meets the rules and regulations of the remote gaming authority that regulates and licenses our operations, First Cagayan Leisure and Resort Corporation (FCRLC) for the Cagayan Economic Zone Authority (CEZA), of Santa Ana, Cagayan, Philippines ;`, tag: 'div.about-desc' },
        ...
        { text: '2. When your computer logs into a remote gambling website, do not leave the computer unattended.', tag: 'div.about-desc' },
        { text: '3. Do not disclose your credit card or bank account details to minors.', tag: 'div.about-desc' },
        { text: '4. Do not enable the "Save Password" option on the our platform login page.', tag: 'div.about-desc' },
        ...
        { text: '6. If you know of someone under the age of 18 (or under the legal age in their jurisdiction) who has mistakenly registered as our player, please notify us immediately.', tag: 'div.about-desc' },
...
//         // 設置 localStorage 中的 token
//         await page.addInitScript(({ key, value }) => {
//             localStorage.setItem(key, value);
//         }, { key: userRecordKey, value: userRecordValue }); // 使用 Q1constants 中的 token

//         globalThis.page = page;
//     });
    

//     test('WEB 登入頁檢查', async () => {
//         // 導航到測試頁面，這裡 token 已經設置好
//         await globalThis.page.goto('http://web-q1-npf2.qit1.net/mine/wallet'); // 修改为 Web URL
...
//             const isVisible = await globalThis.page.locator(element.selector).isVisible();
...
//     test('WEB 其他頁面測試', async () => {
//         // 導航到另一個測試頁面
//         await globalThis.page.goto('https://web-q1-npf2.qit1.net/mine/rebate'); // 修改为另一個 Web URL
//         await globalThis.page.waitForLoadState('networkidle');
